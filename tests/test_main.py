...
@pytest.mark.asyncio
async def test_on_user_message_cancels_timeout(plugin, mock_event):
    # 模拟超时任务
    s = plugin._get_or_create_session(mock_event.unified_msg_origin)
    s.timeout_task = AsyncMock()
    s.timeout_task.cancel = MagicMock()
    await plugin._on_user_message(mock_event)
    s.timeout_task.cancel.assert_called_once()
    assert s.timeout_task is None

@pytest.mark.asyncio
async def test_on_llm_response_starts_timeout(plugin, mock_event):
...
@pytest.mark.asyncio
async def test_scheduler_tick(plugin):
    # 简单测试调度循环，不实际运行
...
@pytest.mark.asyncio
async def test_execute_trigger_plain(plugin):
