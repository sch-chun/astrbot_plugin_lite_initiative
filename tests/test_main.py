...
async def test_daily_analysis(plugin: LiteInitiativePlugin) -> None:
    with patch('main._get_now_tz') as mock_now:
...
async def test_execute_trigger_plain(plugin: LiteInitiativePlugin) -> None:
    ...
    with patch('main.run_trigger', return_value=("hello", True)) as mock_run:
        with patch('main.save_proactive_history', new=AsyncMock()) as mock_save:
